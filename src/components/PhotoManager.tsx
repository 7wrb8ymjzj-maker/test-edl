 ...
 */
const compressImage = (file: File, maxDim: number = 1600, quality: number = 0.75): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onerror = (err) => reject(new Error("Erreur de rendu de l'image."));
    };
...
export default function PhotoManager({ photos = [], onChange, readOnly = false }: PhotoManagerProps) {
  ...
  const processFiles = async (files: FileList | null) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        // Compress to 1600px Max, 0.75 JPEG Quality
