...
export default function HistoryList({
  ...
  const [proposeEmailFor, setProposeEmailFor] = useState<Inspection | null>(null);
  const [proposeEmailForDC, setProposeEmailForDC] = useState<DeliveryControl | null>(null);
  ...
  const handleDownloadPdf = async (e: React.MouseEvent, rec: HistoryRecord) => {
    ...
    if ("isDeliveryControl" in rec && rec.isDeliveryControl === true) {
      ...
      const success = await generateDeliveryControlPDF(rec);
      setLoadingPdfId(null);
      if (success) {
        setProposeEmailForDC(rec);
      }
    } else {
      ...
      const success = await generateInspectionPDF(inspection);
      setLoadingPdfId(null);
      if (success) {
        setProposeEmailFor(inspection);
      }
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-150 shadow-[0_2px_8px_rgba(0,0,0,0.06)] p-6 md:p-8">
          ...
          {filteredRecords.map((rec) => {
            ...
            if (isDC) {
              ...
              return (
                <div
                  key={dc.id}
                  className="group relative flex flex-col justify-between p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-[#D96C0F] hover:shadow-md transition-all bg-white border-l-4 border-l-[#D96C0F] text-left select-none"
                >
                  <div>
                    {/* Header bar row */}
              ...
              return (
                <div
                  key={ins.id}
                  className="group relative flex flex-col justify-between p-5 rounded-2xl border border-slate-150 shadow-sm hover:border-bungeco-orange hover:shadow-md transition-all bg-white text-left select-none"
                >
                  <div>
      ...
      {/* --- EMAIL SENDER MODAL FOR STANDARD EDL --- */}
      {proposeEmailFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-3xs animate-fade-in">
          <div className="bg-white rounded-2xl border border-slate-150 shadow-2xl p-6 max-w-sm w-full font-sans text-left">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-bungeco-orange">
                <Mail size={20} />
              </div>
              <div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">
                  Envoi de l'EDL par e-mail
                </h3>
                <p className="text-[10px] text-slate-400 font-mono tracking-tight font-bold">
                  MATRICULE : {proposeEmailFor.moduleNumber}
                </p>
              </div>
            </div>
            
            <p className="text-slate-600 text-xs leading-relaxed mb-6">
              L'état des lieux du module <strong>{proposeEmailFor.moduleNumber}</strong> a été généré avec succès au format PDF.
              <br /><br />
              Souhaitez-vous envoyer le rapport par e-mail à <strong>contact@bungeco.fr</strong> ?
            </p>
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                onClick={() => {
                  const recipient = "contact@bungeco.fr";
                  const subject = encodeURIComponent(`Rapport EDL ${proposeEmailFor.moduleNumber}`);
                  const body = encodeURIComponent(
                    `Bonjour,\n\nVeuillez trouver ci-joint le rapport d'état des lieux au format PDF pour le module de type ${proposeEmailFor.moduleType}, matricule ${proposeEmailFor.moduleNumber}.\n\nDate du constat : ${proposeEmailFor.date}\n\nCordialement.`
                  );
                  window.location.href = `mailto:${recipient}?subject=${subject}&body=${body}`;
                  setProposeEmailFor(null);
                }}
                className="flex-1 bg-bungeco-orange hover:bg-[#c25e0b] hover:scale-[1.02] active:scale-[0.98] text-white font-bold text-xs py-2.5 px-4 rounded-xl transition-all shadow-md hover:shadow-lg cursor-pointer text-center"
              >
                Oui, envoyer
              </button>
              <button
                type="button"
                onClick={() => setProposeEmailFor(null)}
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-2.5 px-4 rounded-xl transition-all cursor-pointer text-center"
              >
                Fermer
              </button>
            </div>
      ...
      {/* --- EMAIL SENDER MODAL FOR DELIVERY CONTROL COMMANDE --- */}
      {proposeEmailForDC && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-3xs animate-fade-in">
          <div className="bg-white rounded-2xl border border-slate-150 shadow-2xl p-6 max-w-sm w-full font-sans text-left">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-[#1e293b]/10 flex items-center justify-center text-slate-800">
                <Mail size={20} />
              </div>
              <div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider leading-none">
                  Contrôle expédié par e-mail
                </h3>
                <p className="text-[10px] text-slate-400 font-mono tracking-tight font-bold uppercase mt-1">
                  Client : {proposeEmailForDC.client}
                </p>
              </div>
            </div>
            
            <p className="text-slate-600 text-xs leading-relaxed mb-6">
              La synthèse qualité de la commande client <strong>{proposeEmailForDC.client}</strong> ({proposeEmailForDC.modules.length} modules) a été générée sous format PDF multi-pages.
              <br /><br />
              Souhaitez-vous rédiger et envoyer cet e-mail officiel à <strong>contact@bungeco.fr</strong> avec le nom du client en objet ?
            </p>
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                onClick={() => {
                  const recipient = "contact@bungeco.fr";
                  const subject = encodeURIComponent(`Contrôle expédition : Client ${proposeEmailForDC.client}`);
                  const body = encodeURIComponent(
                    `Bonjour,\n\nVeuillez trouver ci-joint le dossier de contrôle d'expédition multi-modules pour la commande client ${proposeEmailForDC.client}.\n\nNombre de modules vérifiés : ${proposeEmailForDC.modules.length}\nDate du contrôle : ${proposeEmailForDC.date}\nContrôleur : ${proposeEmailForDC.controller}\n\nCordialement.`
                  );
                  window.location.href = `mailto:${recipient}?subject=${subject}&body=${body}`;
                  setProposeEmailForDC(null);
                }}
                className="flex-1 bg-bungeco-orange hover:bg-[#c25e0b] hover:scale-[1.02] active:scale-[0.98] text-white font-bold text-xs py-2.5 px-4 rounded-xl transition-all shadow-md hover:shadow-lg cursor-pointer text-center"
              >
                Oui, envoyer
              ...
              <button
                type="button"
                onClick={() => setProposeEmailForDC(null)}
                className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-2.5 px-4 rounded-xl transition-all cursor-pointer text-center"
              >
                Fermer
              </button>
